...
**Important:** After creating the database model, you need to generate and run a Flask migration to create the table in your database.

### Generate Migration

```bash
# Generate a new migration file
flask db migrate -m "Add Building model"
```

This will create a new migration file in `migrations/versions/` with content similar to:
...
def upgrade():
    ...
    sa.Column('gml_file_path', sa.Text(), nullable=False),
    sa.Column('texture_file_path', sa.Text(), nullable=False),
...
psql -h localhost -U appuser -d test_db -c "\d buildings"
```
...
```bash
# If migration fails, check the current migration status
flask db current
...
# If migration gets stuck, you might need to mark it as applied
flask db stamp head
```

**For Testing Environment:**
When running tests, the database tables are created automatically by the test fixtures in `conftest.py`, but for development you must run the migrations manually.
